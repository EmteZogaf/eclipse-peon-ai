...
/**
 * Jon's delegate tools. Each drives one of his own, RAM-only slaves — a Peon-Plan ("Da Thinka") and a
 * Peon-Dev ("Da Mek") instance — for a single turn and returns the slave's reply verbatim as the tool
 * result. Two verbs per slave: a plain question ({@code talkPlan} / {@code askDev}) and the real work
 * ({@code planWithPlanAgent} writes the plan, {@code buildWithAgent} builds it).
 *
 * <p>The two slaves are shared, eager singletons: the layer that wires the tool creates them (core
 * supplies disk-tool slaves, the Eclipse plugin supplies workspace-tool slaves with the memory-write
 * tool filtered out) and hands them in as {@link NamedAgent}s — the very same instances Jon exposes via
 * {@code AiPoAgent.getTeam()} (ADR-0025). Their in-RAM context carries across calls. Slaves use a
 * RAM-only memory (no history file); the durable handover is the plan file (see ADR-0024).
 *
 * <p><b>Shared memory:</b> the slaves only ever <em>read</em> the shared workspace memory — its content
 * is injected into their standing orders before each dispatch. They cannot write it; only Jon curates
 * the shared memory (and thereby steers the other agents).
 *
 * <p><b>Working orders:</b> {@code planWithPlanAgent} injects a plan-writing discipline
 * (plan-write-loop.txt) as a one-shot standing order; {@code buildWithAgent} keeps its {@code planPath}
 * sticky and injects the build discipline (dev-build-loop.txt) — both survive the slave's own
 * compaction. Jon judges "done vs. still working" from the reply and always has the plan reviewed after
 * the build before the Dev slave calls planImplemented to archive it (steered by po-delegation.txt).
 */
public class JonDelegateTool extends AbstractTool {

    /** Build discipline injected into the Dev slave only when it builds from a released plan. */
    private static final String DEV_BUILD_LOOP = PromptLoader.load("dev-build-loop.txt");
    /** Plan-writing discipline injected into the Plan slave when Jon asks it to write the plan. */
    private static final String PLAN_WRITE_LOOP = PeonPaths.resolve(PromptLoader.load("plan-write-loop.txt"));
    ...
    /** Sticky plan path for the Dev slave — survives across dispatches and the slave's compaction. */
    private String devPlanPath;

    public JonDelegateTool(NamedAgent plan, NamedAgent dev, Supplier<List<String>> memoryProvider) {
        this.plan = plan;
        this.dev = dev;
    ...
    public String buildWithAgent(@P(name = "prompt") String prompt,
        ...
        var orders = baseOrders();
        // Hand Da Mek the path AND the build discipline (task-by-task, green gate, compactSession) as its
        // way of working — never the whole plan; the file is the durable handover.
    ...
    /** The Plan slave (Da Thinka), an eager shared instance also exposed via the PO team. */
    public AiAgent getPlanSlave() {
        return plan.agent();
    }

    /** The Dev slave (Da Mek), an eager shared instance also exposed via the PO team. */
    public AiAgent getDevSlave() {
        return dev.agent();
    }

    /** Shared memory + injected context (AGENTS.md, selected project) — read-only for the slaves. */
    private ArrayList<String> baseOrders() {
        return new ArrayList<>(memoryProvider.get());
    }

    private String dispatch(NamedAgent target, String prompt, List<String> orders) {
